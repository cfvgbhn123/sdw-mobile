...
var indexMethods = {
    ...
    requestGameItemList: function () {
        /*简单检测用户token*/
        if (!this.userPayToken && !this.defaultToken) {
        ...
        var postUri = SDW_WEB.URLS.addParam({
            payCode: self.userPayToken || this.defaultToken,
        ...
        SDW_WEB.getAjaxData(postUri, function (res) {
            ...
            if (res.result === 1) {
                self.userPayToken =  self.userPayToken  || self.defaultToken ;
                localStorage.defaultToken = self.userPayToken ;
                self.currentPage = 'itemList';
                self.wxUserInfo = res;
...
var _indexView = new Vue({
    ...
    mounted: function () {
        ...
        if(localStorage.defaultToken){
            this.defaultToken = localStorage.defaultToken;
            setTimeout(function () {
                self.requestGameItemList();
            },0)
        }
