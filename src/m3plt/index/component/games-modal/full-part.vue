...
<template>
        <div  :class="[info.isPlaying?'right-part ':'right-part slide-in-right', {'m3pltGame-right-part': info.isPlaying&&onM3pltGame}]" ref="gameBox">
            ...
            <div :class="['games-container', {'m3pltGame-full-games-container': onM3pltGame}]" ref="gamesCon">
                <full-container
                    :gameInfo="item"
                    :index ="i"
                    :modalInfo="info"  v-for="(item, i) in info.gamePlayList"
                    :key="item.id"
                    :container-width="conWith" v-if="item.screen==2 || item.screen==3">
                </full-container>
...
<script>
    var $=require('jquery');
    var fullContainer= require('./full-container.vue');
